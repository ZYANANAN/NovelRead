...
@property (nonatomic, strong) UIImageView *logoImageView;//added by 王炳聪
...
- (instancetype)initWithFrame:(CGRect)frame{
    if((self = [super initWithFrame:frame])){
        ...
        //UIColor *globalTint = [UINavigationBar appearance].tintColor;
        UIColor *globalTint = [UIColor redColor];
        SVProgressHUDForegroundColor = globalTint;
...
#pragma clang diagnostic pop
    ...
    //更新logoimageview位置，update by 王炳聪
    self.logoImageView.center = self.indefiniteAnimatedView.center;
...
- (void)updateBlurBounds{
    ...
    if([[[UIDevice currentDevice] systemVersion] compare:@"8.0"] != NSOrderedAscending){
        ...
        UIToolbar *bar = [[UIToolbar alloc] initWithFrame:self.hudView.bounds];
        bar.barStyle = UIBarStyleBlack;
        [self.hudView insertSubview:bar atIndex:0];
    }
}
...
- (void)updateMotionEffectForOrientation:(UIInterfaceOrientation)orientation{
    if([_hudView respondsToSelector:@selector(addMotionEffect:)]){
        ...
        self.hudView.motionEffects = nil;
...
- (UIImage*)image:(UIImage*)image withTintColor:(UIColor*)color{
    CGRect rect = CGRectMake(1.0f, 1.0f, image.size.width, image.size.height);
...
- (UIView*)hudView{
    if(!_hudView){
        ...
        //添加中间开卷的icon，add by 王炳聪
        //hudview是100x100，logoview是20x20
        UIImage *logoImage = [UIImage imageNamed:@"loading_logo"];
        UIImage *tempImage = [logoImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        _logoImageView = [[UIImageView alloc] initWithImage:tempImage];
        _logoImageView.tintColor = [UIColor whiteColor];
        _logoImageView.frame = CGRectMake(0, 0, 20, 20);
        _logoImageView.center = CGPointMake(50, 50);
        [_hudView addSubview:_logoImageView];
    }
